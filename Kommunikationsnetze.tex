...
Systemkapazität: $K = N + s$ mit $s$ = Anzahl Pufferplätze (Zustände $0..K$)
\end{sectionbox}
\end{minipage}
...
\begin{sectionbox}
M/M/1/s ($K = s+1$ Zustände, $s$ Pufferplätze):

$$
p_n = p_0 \rho^n, \quad \rho=\frac{\lambda}{\mu}
$$

$$
p_0 = \frac{1-\rho}{1-\rho^{s+1}} \quad (\rho \ne 1)
$$
...
M/M/N/s (Geburts-Tod, $s+1$ Zustände):

$$
p_{n+1} = p_n \cdot \frac{\lambda}{\mu \min(n+1,N)} \quad (n \le s)
$$

Normierung:

$$
p_0 = \left(\sum_{n=0}^{s} \prod_{i=0}^{n-1} \frac{\lambda}{\mu\min(i+1,N)}\right)^{-1}
$$
